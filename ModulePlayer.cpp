...
void ModulePlayer::CreateVehicle()
{
	...
	// FRONT-LEFT ------------------------
	car.wheels[0].connection.Set(half_width + 0.4f * wheel_width, connection_height, half_length - wheel_radius);
...
btVector3 ModulePlayer::CarPos()
{
	...
	btVector3 vec = vehicle->vehicle->getChassisWorldTransform().getOrigin();
	return vec;

}

vec2 ModulePlayer::CarMovement()
{

	vec2 carPos;

	btQuaternion quat = vehicle->vehicle->getChassisWorldTransform().getRotation();

	quat.normalize();

	double sinn = (1 + 2 * (quat.getW() * quat.getY() - quat.getX() * quat.getZ()));
	double coss = (1 - 2 * (quat.getW() * quat.getY() - quat.getX() * quat.getZ()));
	sinn = sinn / coss;
	sinn = sqrt(sinn);
	float carMov = 2 * atan(sinn) - M_PI / 2;
